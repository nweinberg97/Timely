...
window.jumpToCurrentRealWorldDate = function() {
    state.currentDate = new Date(); // Anchor today back precisely to true system clock
    renderApp();
};
...
function renderDailyView(target) {
    ...
    const dateIsoKey = d.toISOString().split('T')[0]; // Yields unique "YYYY-MM-DD"
    
    const isRealWorldToday = (
        d.getFullYear() === todayRealWorld.getFullYear() && 
        d.getMonth() === todayRealWorld.getMonth() && 
        d.getDate() === todayRealWorld.getDate()
    );
    const realWorldHour = todayRealWorld.getHours();

    let html = generateTimeNavigatorHTML("Daily Schedule", formattedDateStr);
    html += `<div style="display:flex; justify-content:flex-end; margin-bottom:1rem;"><button class="add-card-btn" onclick="createNewCardFromUI('Daily Card', '${dateIsoKey}_08:00')">+ Add Time Block</button></div>`;
    ...
    hours.forEach(hour => {
        ...
        const uniqueSlotId = `${dateIsoKey}_${hour}`;

        html += `
            <div class="time-slot-row ${todayClassModifier}">
                <div class="slot-time-label">${hour}</div>
                <div class="slot-drop-zone drop-zone" data-zone-type="daily" data-zone-id="${uniqueSlotId}">
                    ${renderCardsForZone('daily', uniqueSlotId)}
                </div>
            </div>
        `;
    });
...
function renderWeeklyView(target) {
    ...
    days.forEach((day, index) => {
        ...
        const dayIsoKey = evaluatedDayDate.toISOString().split('T')[0]; // Explicit date key for this explicit day column
        
        const isWeeklyColumnToday = (
            evaluatedDayDate.getFullYear() === todayRealWorld.getFullYear() && 
            evaluatedDayDate.getMonth() === todayRealWorld.getMonth() && 
            evaluatedDayDate.getDate() === todayRealWorld.getDate()
        );
        const columnClassModifier = isWeeklyColumnToday ? 'is-today' : '';

        html += `<div class="weekly-day-col ${columnClassModifier}"><div class="day-col-header">${day} (${evaluatedDayDate.getDate()})</div>`;
        sections.forEach(sec => {
            const compositeKey = `${dayIsoKey}_${sec}`;
            html += `
...
function renderMonthlyView(target) {
    ...
    for (let dayCounter = 1; dayCounter <= totalDaysInActiveMonth; dayCounter++) {
        const cellDate = new Date(d.getFullYear(), d.getMonth(), dayCounter);
        const cellIsoKey = cellDate.toISOString().split('T')[0]; // Unique key scoped to that exact year-month-day

        const isMonthlyCellToday = (
            cellDate.getFullYear() === todayRealWorld.getFullYear() && 
            cellDate.getMonth() === todayRealWorld.getMonth() && 
            dayCounter === todayRealWorld.getDate()
        );
        ...
        html += `
            <div class="monthly-day-cell ${cellClassModifier}">
                ...
                <div class="drop-zone" style="flex-grow:1;" data-zone-type="monthly" data-zone-id="${cellIsoKey}">
                    ${renderCardsForZone('monthly', cellIsoKey)}
                </div>
            </div>
        `;
    }
            ...
            <div class="flex-board-column animate-column-entry">
                <div class="pane-header">
                    <div style="display: flex; align-items: center; gap: 0.5rem; flex-grow: 1;">
                        <button class="card-btn" onclick="deletePlanningContainer('${item.id}')" title="Delete Column" style="font-size: 0.85rem; padding: 0.1rem 0.3 rem; color: var(--trash-text);">✕</button>
